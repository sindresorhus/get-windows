	...
	try {
		...
		if (result && result.length > 0) {
			for (const openWindow of result) {
				if (openWindows.has(openWindow.owner.bundleId)) {
					openWindows.get(openWindow.owner.bundleId).processes({...openWindow, platform: 'macos'});
				} else {
					openWindows.set(openWindow.owner.bundleId, {processes: [{...openWindow, platform: 'macos'}]});
				}
			}
		}
