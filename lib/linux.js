...
module.exports.getOpenWindows = async () => {
	try {
		...
		for await (const windowId of windowsIds) {
			const windowInformation = await getWindowInformation(Number.parseInt(windowId, 16));
			if (openWindows.has(windowInformation.owner.name)) {
				openWindows.get(windowInformation.owner.name).push(windowInformation);
			} else {
				openWindows.set(windowInformation.owner.name, [windowInformation]);
			}
		}
...
module.exports.getOpenWindowsSync = () => {
	try {
		...
		for (const windowId of windowsIds) {
			...
			if (openWindows.has(windowInformation.owner.name)) {
				openWindows.get(windowInformation.owner.name).processes.push(windowInformation);
			} else {
				openWindows.set(windowInformation.owner.name, {processes: [windowInformation]});
			}
		}
