...
async function getWindowInformation(windowId) {
	const [{stdout}, {stdout: boundsStdout}] = await Promise.all([
		execFile(xpropBin, [...xpropDetailsArgs, windowId]),
...
function getWindowInformationSync(windowId) {
	const stdout = childProcess.execFileSync(xpropBin, [...xpropDetailsArgs, windowId], {encoding: 'utf8'});
...
module.exports.getOpenWindows = async () => {
	try {
		...
		for (let i = 0;i < windowsIds.length; i++) {
			const windowInformation = await getWindowInformation(Number.parseInt(windowsIds[i], 16));
			openWindows.set(windowInformation.title, {processes: [windowInformation]});
		}
		return openWindows;
	} catch {
		return undefined;
...
module.exports.getOpenWindowsSync = () => {
	try {
		...
		for (let i = 0;i < windowsIds.length; i++) {
			const windowInformation = getWindowInformationSync(Number.parseInt(windowsIds[i], 16));
			openWindows.set(windowInformation.title, {processes: [windowInformation]});
		}
		return openWindows;
	} catch (error) {
