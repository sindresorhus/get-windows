...
const BRAVE = 'brave.exe';
const browserList = [CHROME, FIREFOX, EDGE, BRAVE];

let ActiveURL;

try {
	let _path = path.join(__dirname, '../../../../../dlls/ActiveURL.dll');

	if (!fs.existsSync(_path)) {
		_path = path.join(__dirname, '../../../../dlls/ActiveURL.dll');
	}
...
function windows() {
	...
	let windowURL;
	let windowIncognito;
	const constructChromeAndBraveUrl = (url, browserScheme) => {
		const isEmpty = url.length === 0;
		const isBrowserScheme = url.startsWith(browserScheme);
		const isFilePath = !isEmpty && url[0] === url[0].toUpperCase();
		const hasWhiteSpace = /\s/g.test(url);
		const isHttpURL = !isEmpty && !isBrowserScheme && !isFilePath && !hasWhiteSpace;

		if (!isHttpURL) {
			return '';
		}

		const isHttpScheme = isHttpURL && url.startsWith('http');
	...
	try {
		if (ActiveURL && browserList.includes(processName.toLowerCase())) {
			switch (processName.toLowerCase()) {
				case BRAVE: {
					const { url, incognito } = JSON.parse(ActiveURL.GetBraveDetails(activeWindowHandle));
					windowURL = constructChromeAndBraveUrl(url, 'brave://');
					windowIncognito = incognito;
					break;
				}

				case CHROME: {
					const { url, incognito } = JSON.parse(ActiveURL.GetChromeDetails(activeWindowHandle));
					windowURL = constructChromeAndBraveUrl(url, 'chrome://');
					windowIncognito = incognito;
					break;
				}

				case FIREFOX: {
					const { url, incognito } = JSON.parse(ActiveURL.GetFirefoxDetails(activeWindowHandle));
					const isEmpty = url.length === 0;
					const hasWhiteSpace = /\s/g.test(url);
					windowURL = (isEmpty || hasWhiteSpace) ? '' : url;
					windowIncognito = incognito;
					break;
				}

				case EDGE: {
					const { url, incognito } = JSON.parse(ActiveURL.GetEdgeDetails(activeWindowHandle));
					const isEmpty = url.length === 0;
					const hasWhiteSpace = /\s/g.test(url);
					windowURL = (isEmpty || hasWhiteSpace) ? '' : url;
					windowIncognito = incognito;
