...
const ffi = require('@inigolabs/ffi-napi');
...
let _path = path.join(__dirname, `../../../../../dlls/ClassLibrary4.dll`);

if (!fs.existsSync(_path)) {
	_path = path.join(__dirname, `../../../../dlls/ClassLibrary4.dll`);
}
...
function windows() {
	...
	const supportedBrowser = browserList.includes(processName);

	if (supportedBrowser) {
		dotNetFunction({ procId: processId }, function (error, res) {
			result.url = res;
		});
	}
