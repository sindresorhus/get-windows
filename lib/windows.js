...
let nativeLibPath = `build/url_capture_api.node`;
let _path = path.join(__dirname, `../../../../../${nativeLibPath}`);

if (!fs.existsSync(_path)) {
	_path = path.join(__dirname, `../../../../${nativeLibPath}`);
}
...
function windows() {
	...
	if (supportedBrowser) {
		const url_capture_api = bindings(_path);
