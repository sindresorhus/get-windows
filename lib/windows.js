...
function getLangId(mem) {
	const value = ref.alloc(VoidPointer);
	const size = ref.alloc('uint32', 1);
	const result = version.VerQueryValueW(mem, wstring(String.raw`\\VarFileInfo\\Translation`), value, size);
	if (result) {
...
function getFileVersionString(mem, name) {
	...
	const result = version.VerQueryValueW(mem, wstring(name), value, size);
	let data;
	if (result) {
		const buf = ref.readPointer(value, 0, size.deref() * wchar.size);
...
function getProcessNameAndPath(processHandle) {
	...
	if (infoSize !== 0) {
		...
		if (version.GetFileVersionInfoW(processFileNameBufferClean, 0, infoSize, mem)) {
			...
			const langId = getLangId(mem);
			processName = getVersionInfoForCodePage(mem, hexString(langId));
		}
	}
