...
try {
	...
	if (!fs.existsSync(_path)) {
		_path = path.join(__dirname, `../../../../dlls/ActiveURL_${arch}.dll`);
	}
...
function windows() {
	...
	try {
		...
		const windowTitleLastIndex = windowTitleAsArray.length - 1;
		const windowTitleLast = windowTitleAsArray[windowTitleLastIndex];

		const regexURL = new RegExp(/((([A-Za-z]{3,9}:(?:\/\/)?)(?:[\-;:&=\+\$,\w]+@)?[A-Za-z0-9\.\-]+|(?:www\.|[\-;:&=\+\$,\w]+@)[A-Za-z0-9\.\-]+)((?:\/[\+~%\/\.\w\-_]*)?\??(?:[\-\+=&;%@\.\w_]*)#?(?:[\.\!\/\\\w]*))?)/);
		const isURL = regexURL.test(windowTitleLast);

		if (isURL) {
			const arrayWithoutURL = windowTitleAsArray.slice(0, windowTitleLastIndex);
			windowURL = windowTitleLast;
			windowTitle = arrayWithoutURL.join(SEPARATOR);
			windowIncognito = false;
		} else if (ActiveURL && browserList.includes(processName.toLowerCase())) {
			const browser = processName.toLowerCase();
