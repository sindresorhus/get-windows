...
function getEnumWindows() {
	...
	const windowProcess = ffi.Callback('bool', ['pointer', 'int32'], hwnd => {
		...
		if (user32.IsWindowVisible(hwnd)) {
			...
			/** If window information is not undefined */
			if (openWindows.has(winInfo.owner.name)) {
				openWindows.get(winInfo.owner.name).processes.push(winInfo);
			} else {
				openWindows.set(winInfo.owner.name, {processes: [winInfo]});
			}
		} else {
			return false;
		}
