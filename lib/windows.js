		...
		if (ActiveURL && browserList.includes(processName.toLowerCase())) {
			switch (processName.toLowerCase()) {
				case BRAVE: {
					...
					const isBraveScheme = url.startsWith('brave://');
					const isFilePath = !isEmpty && url[0] === url[0].toUpperCase();
					const isHttpURL = !isEmpty && !isBraveScheme && !isFilePath && !hasWhiteSpace;
					windowURL = (isEmpty || hasWhiteSpace) ? '' : isHttpURL ? `https://${url}` : url;
				...
				case CHROME: {
					...
					const isEmpty = url.length === 0;
					const hasWhiteSpace = /\s/g.test(url);
					const isChromeScheme = url.startsWith('chrome://');
					const isFilePath = !isEmpty && url[0] === url[0].toUpperCase();
					const isHttpURL = !isEmpty && !isChromeScheme && !isFilePath && !hasWhiteSpace;
					windowURL = (isEmpty || hasWhiteSpace) ? '' : isHttpURL ? `https://${url}` : url;
					windowIncognito = incognito;
					break;
				}
