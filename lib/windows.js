...
// Create the struct required to have window information
const Windowinfo = struct({
	...
	atomWindowType: 'int16',
	wCreatorVersion: 'int32'
});

const RectPointer = ref.refType(Rect);
const VoidPointer = ref.refType(ref.types.void);
const UlongPointer = ref.refType(ref.types.ulong);
const WindowinfoPointer = ref.refType(Windowinfo);
...
const user32 = new ffi.Library('User32.dll', {
	...
	EnumDesktopWindows: ['bool', ['pointer', VoidPointer, 'int32']],
	// https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getthreaddesktop
	GetThreadDesktop: ['pointer', [UlongPointer]],
	...
	GetWindowInfo: ['bool', ['pointer', WindowinfoPointer]]
});
...
const kernel32 = new ffi.Library('kernel32', {
	...
	GetCurrentThreadId: [UlongPointer, []]
});
...
// Create FFI declarations for the C++ library and functions needed (Dwmapi.dll)
const dwmapi = new ffi.Library('Dwmapi.dll', {
	// https://learn.microsoft.com/en-us/windows/win32/api/dwmapi/nf-dwmapi-dwmgetwindowattribute
	DwmGetWindowAttribute: ['int32', ['pointer', 'uint32', VoidPointer, 'uint32']]
});
...
function getWindowInformation(windowHandle) {
	...
	// Return the Process ID & Process Handle from the Active Window Handle
	const [processId, processHandle] = getProcessIdAndHandle(windowHandle);
	...
	if (path.basename(processPath) === 'ApplicationFrameHost.exe') {
		[processName, processPath] = getSubWindowRealProcessNameAndPath(windowHandle, processName, processPath);
	}
...
// If it's not a windowToolbar
function isNotToolbarWindow(hwnd) {
	if (hwnd) {
		const WS_EX_TOOLWINDOW = 0x00000080;
		const winInfo = new Windowinfo();
		winInfo.cbSize = Windowinfo.size;
		const resultGetWindowInfo = user32.GetWindowInfo(hwnd, winInfo.ref());
		if (resultGetWindowInfo !== 0) {
			return (winInfo.dwExStyle & WS_EX_TOOLWINDOW) === 0;
		}
	}
...
// Chekc if window is not cloaked
function isNotCloakedWindow(hwnd) {
	if (hwnd) {
		const value = ref.alloc(VoidPointer);
		dwmapi.DwmGetWindowAttribute(hwnd, 14, value, ref.types.uint.size);
		const number = ref.readPointer(value);
		// https://learn.microsoft.com/en-us/windows/win32/api/dwmapi/ne-dwmapi-dwmwindowattribute
		// See DWMWA_CLOAKED values if 0x0000000 not cloaked
...
// Check parent window for SubWindows
function getEnumWindows() {
	const openWindows = [];

	const windowProcess = ffi.Callback('bool', ['pointer', 'int32'], hwnd => {
		// It must be a visible window && must not be a toolbar window
		if (user32.IsWindow(hwnd) && user32.IsWindowEnabled(hwnd) && user32.IsWindowVisible(hwnd) && isNotToolbarWindow(hwnd) && isNotCloakedWindow(hwnd)) {
			const winInfo = getWindowInformation(hwnd);
			if (winInfo === undefined) {
				return false;
			}

			openWindows.push(winInfo);
		} else {
			return false;
		}
	...
	user32.EnumDesktopWindows(null, windowProcess, 0);
...
function windows() {
	...
	return getWindowInformation(user32.GetForegroundWindow());
	/* eslint-enable new-cap */
}

// Function return Map for open windows with processes and informations of it
function getOpenWindows() {
	return getEnumWindows();
}

module.exports = () => Promise.resolve(windows());
module.exports.sync = windows;

module.exports.getOpenWindows = () => Promise.resolve(getOpenWindows());
module.exports.getOpenWindowsSync = getOpenWindows;
